...
function DecodeRGBA(const S: string; var Opa:single): Integer;
var
  RS, GS, BS, OP: string;
  RGB: string;
  R, B, G: Integer;
begin
  Result := SVG_INHERIT_COLOR;
  if not ((Copy(S, 1, 5) = 'rgba(') and (S[Length(S)] = ')')) then
    Exit;

  RGB := Copy(S, 6, Length(S) - 6);
  RGB := Trim(RGB);
  ...
  Opa := DecodeSVGColorToInt(OP) / 255;
...
begin
  ...
  begin
    if pos('rgba(',Color)>0 then
      Result := DecodeRGBA(Color, Alpha)
    else
